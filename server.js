...
function verificarDisponibilidade(quartoId, categoria, checkIn, checkOut) {
  ...
  try {
    ...
    // Como cada categoria é um único quarto, verificamos por categoria
...
app.get('/api/quartos/:categoria/disponibilidade', async (req, res) => {
  try {
    ...
    // Buscar todas as reservas ativas do quarto
    const reservas = queryAll(`
      SELECT * FROM reservas 
      WHERE categoria = ?
      AND status NOT IN ('Concluído', 'Cancelado')
...
app.get('/api/quartos/:categoria/datas-livres', async (req, res) => {
  try {
    ...
    const disponibilidade = verificarDisponibilidade(null, categoria, check_in, check_out);
    
    if (!disponibilidade.disponivel) {
      // Buscar todas as reservas para calcular datas livres
      const reservas = queryAll(`
